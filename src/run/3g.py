...
def train(
    ...
    for epoch in range(args.n_epochs):
        ...
        logger.info(f'Scores saved for epoch {epoch+1}')
        
        # Track best validation accuracy
        current_acc = valid_logs.get('valid_accuracy', 0)
        if not hasattr(train, 'best_valid_acc'):
            train.best_valid_acc = -1


        if rank == 0 and current_acc > train.best_valid_acc:
            train.best_valid_acc = current_acc
            ...
            best_score_path = os.path.join(checkpoint_dir, 'best_score.json')
            with open(best_score_path, 'w') as f:
                json.dump(score, f, indent=4)
            logger.info(f'New best model saved at epoch {epoch+1} with accuracy {current_acc:.4f}')
        map_location = f'cuda:{rank}' if torch.cuda.is_available() else 'cpu'
        state_dict = torch.load(best_checkpoint_path, map_location=map_location)
        model.load_state_dict(state_dict['model'])
        logger.info(f'Checkpoint loaded from {best_checkpoint_path}')
