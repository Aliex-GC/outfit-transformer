...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, choices=['original', 'clip'],
                        default='clip')
    parser.add_argument('--complementary_checkpoint', type=str, 
                        default='checkpoints/complementary_clip_best.pth', help="Checkpoint for complementary")
    parser.add_argument('--compatibility_checkpoint', type=str,
                        default='checkpoints/compatibillity_clip_best.pth', help="Checkpoint for search model")
    parser.add_argument('--polyvore_dir', type=str,
                        default='./datasets/polyvore', help="Directory for Polyvore dataset")
...
def run(args):
    ...
    # 加载兼容性分数模型
    compatibility_model = load_model(
        model_type=args.model_type,  # 使用统一的 model_type
        checkpoint=args.compatibility_checkpoint  # 指定兼容性模型的检查点
    )
    compatibility_model.eval()

    # 加载互补物品搜索模型
    complementary_model = load_model(
        model_type=args.model_type,  # 使用统一的 model_type
        checkpoint=args.complementary_checkpoint  # 指定互补物品搜索模型的检查点
    )
    complementary_model.eval()
    ...
    with gr.Blocks() as demo:
        ...
        def compute_score():
            ...
            s = compatibility_model.predict_score(
                query= [query],
